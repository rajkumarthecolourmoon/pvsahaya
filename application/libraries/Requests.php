            ...
            if(substr($ea[$i],-3)=="__j"){
               ...
               // echo "<pre/>";
                ...
                if(!is_array($_FILES[$se]['name'])){
                    ...
                    //print_r($imagedata);
                    //echo $imagedata['file_name'];
                    if($this->ci->upload->display_errors('<p>', '</p>')){
                        echo $this->ci->upload->display_errors('<p>', '</p>');
                        echo '<br/>Please press back button to go back and upload again..';
                        die;
                    }
...
//	echo "<pre>";
    ...
    public function time_stamps($created_at=FALSE, $updated_at=FALSE){
        ...
        if($created_at==TRUE) $this->ci->db->set('created_at',date('Y-m-d H:i:s'));

        if($updated_at==TRUE) $this->ci->db->set('updated_at',date('Y-m-d H:i:s'));
